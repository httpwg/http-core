...
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:date="http://exslt.org/dates-and-times" xmlns:ed="http://greenbytes.de/2002/rfcedit" xmlns:exslt="http://exslt.org/common" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:myns="mailto:julian.reschke@greenbytes.de?subject=rfc2629.xslt" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:saxon="http://saxon.sf.net/" xmlns:saxon-old="http://icl.com/saxon" xmlns:svg="http://www.w3.org/2000/svg" xmlns:x="http://purl.org/net/xml2rfc/ext" xmlns:xi="http://www.w3.org/2001/XInclude" xmlns:xhtml="http://www.w3.org/1999/xhtml" version="2.0" exclude-result-prefixes="date ed exslt fo msxsl myns rdf saxon saxon-old svg x xi xhtml">
...
<xsl:variable name="parsedMaxwidth">
  <xsl:choose>
    ...
    <xsl:when test="$xml2rfc-ext-maxwidth='none'"/>
    <xsl:otherwise>
...
<xsl:param name="outputExtension" select="'xhtml'"/>
...
<xsl:key name="index-first-letter" match="iref|reference" use="translate(substring(concat(/rfc/back/displayreference[@target=current()/@anchor]/@to,@anchor,@item),1,1),'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')"/>

<xsl:key name="index-item" match="iref" use="@item"/>

<xsl:key name="index-item-subitem" match="iref" use="concat(@item,'..',@subitem)"/>

<xsl:key name="index-xref-by-sec" match="xref[@x:sec]|relref[@section]" use="concat(@target,'..',@x:sec,@section)"/>

<xsl:key name="index-xref-by-anchor" match="xref[@x:rel]|relref[@relative]" use="concat(@target,'..',@x:rel,@relative)"/>

<xsl:key name="anchor-item" match="//*[@anchor]" use="@anchor"/>

<xsl:key name="xref-item" match="//xref|//relref" use="@target"/>
...
<!-- IPR version switch -->
<xsl:variable name="ipr-rfc3667" select="(   number($rfcno) &gt; 3708) or   not(     (/rfc/@ipr = 'full2026') or     (/rfc/@ipr = 'noDerivativeWorks2026') or     (/rfc/@ipr = 'noDerivativeWorksNow') or     (/rfc/@ipr = 'none') or     (/rfc/@ipr = '') or     not(/rfc/@ipr)   )"/>
...
<xsl:variable name="ipr-rfc4748" select="(   $ipr-rfc3667 and     ( number($rfcno) &gt;= 4715 and ( number($rfcno)!=4718 and number($rfcno)!=4735 and number($rfcno)!= 4749 ))     or     ( number($rfcno)=4578 or number($rfcno)=4582 or number($rfcno)=4583 or number($rfcno)=4628 or number($rfcno)=4629 or number($rfcno)=4639 or number($rfcno)=4651 or number($rfcno)=4682 or number($rfcno)=4684 or number($rfcno)=4695 or number($rfcno)=4696 )     or     ( not($is-rfc) and $pub-yearmonth &gt;= 200611)   )"/>

<xsl:variable name="ipr-2007-08" select="(   $ipr-rfc4748 and     (       (number($rfcno) &gt; 5000         and number($rfcno) != 5020         and number($rfcno) != 5021         and number($rfcno) != 5034         and number($rfcno) != 5052         and number($rfcno) != 5065         and number($rfcno) != 5094) or       ($xml2rfc-ext-pub-year &gt;= 2008) or       (not($is-rfc) and $pub-yearmonth &gt;= 200709)     )   )"/>

<xsl:variable name="ipr-2008-11" select="(     $is-rfc and $pub-yearmonth &gt;= 200811   )   or   (     /rfc/@ipr = 'trust200811' or     /rfc/@ipr = 'noModificationTrust200811' or     /rfc/@ipr = 'noDerivativesTrust200902' or     /rfc/@ipr = 'trust200902' or     /rfc/@ipr = 'noModificationTrust200902' or     /rfc/@ipr = 'noDerivativesTrust200902' or     /rfc/@ipr = 'pre5378Trust200902'   )"/>
...
<xsl:variable name="ipr-2009-09" select="(     ( not($is-rfc) and $pub-yearmonth &gt;= 200911 )     or     (       $is-rfc and $pub-yearmonth &gt;= 200909 and       $rfcno!=5582 and $rfcno!=5621 and $rfcno!=5632 and $rfcno!=5645 and $rfcno!=5646 and $rfcno!=5681     )   )"/>

<!-- this makes the Jan 2010 TLP text depend on the publication date to be >= 2010-04
     for IDs, and around 2010-01 for RFCs-->
<xsl:variable name="ipr-2010-01" select="(     ( not($is-rfc) and $pub-yearmonth &gt;= 201004 )     or     (       $is-rfc and ($pub-yearmonth &gt;= 201001 or       $rfcno=5741 or $rfcno=5742 or $rfcno=5743)     )   )"/>

<!-- see http://mailman.rfc-editor.org/pipermail/rfc-interest/2009-June/001373.html -->
<!-- for IDs, implement the change as 2009-11 -->
<xsl:variable name="abstract-first" select="(     ($is-rfc and $pub-yearmonth &gt;= 200907)     or     (not($is-rfc) and $pub-yearmonth &gt;= 200911)   )"/>

<!-- RFC 7322 changed the placement of notes -->
<xsl:variable name="notes-follow-abstract" select="(     ($is-rfc and $rfcno &gt;= 7200)     or     ($pub-yearmonth &gt;= 201409)   )"/>

<!-- funding switch -->
<xsl:variable name="funding0" select="(   $rfcno &gt; 2499) or   (not($is-rfc) and /rfc/@docName and $xml2rfc-ext-pub-year &gt;= 1999   )"/>

<xsl:variable name="funding1" select="(   $rfcno &gt; 4320) or   (not($is-rfc) and /rfc/@docName and $xml2rfc-ext-pub-year &gt;= 2006   )"/>
...
<xsl:template name="insert-begin-code">
  <xsl:if test="(self::artwork and @x:is-code-component='yes') or (self::sourcecode and @markers='true')">
    <pre xmlns="http://www.w3.org/1999/xhtml" class="ccmarker cct">
...
<xsl:template name="insert-end-code">
  <xsl:if test="(self::artwork and @x:is-code-component='yes') or (self::sourcecode and @markers='true')">
    <pre xmlns="http://www.w3.org/1999/xhtml" class="ccmarker ccb">&lt;CODE ENDS&gt;</pre>
  </xsl:if>
</xsl:template>
...
<xsl:template name="text-in-artwork">
  ...
  <xsl:choose>
    <xsl:when test="contains($content,'&#x9;')">
      ...
      <span xmlns="http://www.w3.org/1999/xhtml" class="error" title="HTAB character">␉</span>
      <xsl:call-template name="text-in-artwork">
...
<xsl:template match="artwork[xi:include]" priority="9">
  <xsl:variable name="resolved">
    <xsl:element namespace="" name="artwork">
      <xsl:copy-of select="@*"/>
...
<xsl:template match="artwork[@src and starts-with(@type,'image/') or @type='svg']|artwork[svg:svg]">
  ...
  <div xmlns="http://www.w3.org/1999/xhtml" class="{normalize-space($class)}">
    <xsl:choose>
      <xsl:when test="parent::artset">
        <xsl:for-each select="..">
          <xsl:call-template name="attach-paragraph-number-as-id"/>
...
<xsl:template name="emit-postal-line">
  ...
  <xsl:if test="normalize-space($value)!='' or $values">
    ...
    <xsl:choose>
      <xsl:when test="$values">
        <xsl:for-each select="exslt:node-set($values)/*">
          <xsl:choose>
            <xsl:when test="@href">
              <a xmlns="http://www.w3.org/1999/xhtml" href="{@href}"><xsl:value-of select="normalize-space(.)"/></a>
            </xsl:when>
            <xsl:otherwise>
      ...
      <xsl:when test="$link!=''">
        <a xmlns="http://www.w3.org/1999/xhtml" href="{$link}"><xsl:value-of select="normalize-space($value)"/></a>
      </xsl:when>
      <xsl:otherwise>
    ...
    <xsl:if test="$annotation!=''">
      <xsl:text> </xsl:text>
      <i xmlns="http://www.w3.org/1999/xhtml"><xsl:value-of select="$annotation"/></i>
    </xsl:if>
  </xsl:if>
</xsl:template>
...
<xsl:template name="emit-author">
  ...
  <xsl:if test="@x:annotation">
    <xsl:text> </xsl:text>
    <i xmlns="http://www.w3.org/1999/xhtml"><xsl:value-of select="@x:annotation"/></i>
  </xsl:if>
...
<xsl:template match="figure">
  ...
  <xsl:if test="@anchor!=''">
    ...
    <xsl:element namespace="http://www.w3.org/1999/xhtml" name="{$anch-container}">
  ...
  <xsl:if test="(@title!='' or name) or (@anchor!='' and not(@suppress-title='true'))">
    ...
    <p xmlns="http://www.w3.org/1999/xhtml" class="figure">
...
<xsl:template match="front">
  ...
  <xsl:if test="$published-as-rfc">
    <p xmlns="http://www.w3.org/1999/xhtml" class="{$css-publishedasrfc}">
...
<xsl:template match="note">
  ...
  <section xmlns="http://www.w3.org/1999/xhtml" class="{normalize-space($classes)}">
    <xsl:call-template name="copy-anchor"/>
...
<xsl:template match="reference">
  ...
  <dt xmlns="http://www.w3.org/1999/xhtml" id="{@anchor}">
    <xsl:call-template name="insertInsDelClass"/>
    <xsl:variable name="del-node" select="ancestor::ed:del"/>
    <xsl:variable name="rep-node" select="ancestor::ed:replace"/>
    <xsl:variable name="deleted" select="$del-node and ($rep-node/ed:ins)"/>
    ...
    <xsl:if test="not($si) and x:source/@href">
      <xsl:variable name="derivedsi" myns:namespaceless-elements="xml2rfc">
        <xsl:if test="document(x:source/@href)/rfc/@docName">
          <seriesInfo xmlns="" name="Internet-Draft" value="{document(x:source/@href)/rfc/@docName}"/>
        </xsl:if>
        <xsl:if test="document(x:source/@href)/rfc/@number">
          <seriesInfo xmlns="" name="RFC" value="{document(x:source/@href)/rfc/@number}"/>
        </xsl:if>
      </xsl:variable>
...
<xsl:template match="referencegroup">
  ...
  <xsl:if test="@target">
    <dd xmlns="http://www.w3.org/1999/xhtml">&lt;<a href="{@target}"><xsl:value-of select="@target"/></a>&gt;</dd>
  </xsl:if>
</xsl:template>
...
<xsl:template match="references">
  ...
  <xsl:choose>
    ...
    <xsl:when test="not(preceding::references) and $refseccount!=1">
      ...
      <section xmlns="http://www.w3.org/1999/xhtml" id="{$anchor-pref}references">
...
<xsl:template match="section|appendix">
  ...
  <xsl:variable name="sectionNumber">
    <xsl:choose>
      <xsl:when test="ancestor::boilerplate"/>
  ...
  <xsl:if test="$xml2rfc-ext-insert-metadata='yes' and ($is-rfc or $is-submitted-draft) and @anchor='rfc.status'">
    <aside xmlns="http://www.w3.org/1999/xhtml" id="{$anchor-pref}meta" class="{$css-docstatus}"/>
  </xsl:if>
...
<xsl:template name="insert-errata">
  ...
  <xsl:if test="$es">
    <aside xmlns="http://www.w3.org/1999/xhtml" class="{$css-erratum}">
...
<xsl:template match="spanx[@style='verb' or @style='vbare']|tt">
  <span xmlns="http://www.w3.org/1999/xhtml" class="tt">
    <xsl:call-template name="copy-anchor"/>
    <xsl:apply-templates/>
  </span>
</xsl:template>
...
<xsl:template name="xref-to-section">
  ...
  <xsl:choose>
    <xsl:when test="$child-nodes and not($from/@format='none') and $xml2rfc-ext-xref-with-text-generate!='nothing'">
      <xsl:apply-templates select="$child-nodes"/>
      <xsl:text> (</xsl:text>
      <a xmlns="http://www.w3.org/1999/xhtml" href="#{$from/@target}">
        <xsl:if test="$irefs">
...
<xsl:template name="emit-link">
  ...
  <xsl:choose>
    <xsl:when test="$element!=''">
      <xsl:element namespace="http://www.w3.org/1999/xhtml" name="{$element}">
        <xsl:if test="$target!=''">
        ...
        <xsl:choose>
          <xsl:when test="$element!='cite' and $citation-title!=''">
            <cite xmlns="http://www.w3.org/1999/xhtml" title="{$citation-title}">
              <xsl:choose>
                <xsl:when test="$child-nodes">
                  <xsl:apply-templates select="$child-nodes"/>
                </xsl:when>
...
<xsl:template match="xref|relref">
  ...
  <xsl:variable name="useTextContent" select="normalize-space($textContent)!='' and not(@x:rel or @x:sec or @section) and not(self::relref)"/>
...
<xsl:template match="*">
  ...
  <span xmlns="http://www.w3.org/1999/xhtml" class="tt {$css-error}">&lt;<xsl:value-of select="name()"/>&gt;</span>
...
<xsl:template name="insertAuthors">
  ...
  <xsl:if test="$number!='suppress' and $xml2rfc-authorship!='no'">
    ...
    <section xmlns="http://www.w3.org/1999/xhtml" id="{$anchor-pref}authors" class="avoidbreakinside">
      <xsl:call-template name="insert-conditional-pagebreak"/>
      <h2>
        <xsl:if test="$number != ''">
...
<xsl:if test="$xml2rfc-ext-insert-metadata='yes' and ($is-rfc or $is-submitted-draft)"><script xmlns="http://www.w3.org/1999/xhtml">
<xsl:if test="$rfcno!=''">
...
<script xmlns="http://www.w3.org/1999/xhtml" src="{$prettyprint-script}"/></xsl:if><xsl:if test="contains($prettyprint-script,'prettify') and (//artwork[contains(@type,'abnf')] or //sourcecode[contains(@type,'abnf')])">
<script xmlns="http://www.w3.org/1999/xhtml">try {
...
<style xmlns="http://www.w3.org/1999/xhtml" title="rfc2629.xslt">
...
<xsl:template name="insert-index-navigation">
  <p xmlns="http://www.w3.org/1999/xhtml" class="{$css-noprint}">
...
<xsl:template name="insert-index-item">
  ...
  <xsl:choose>
    <xsl:when test="$in-artwork">
      <span xmlns="http://www.w3.org/1999/xhtml" class="tt"><xsl:value-of select="@item"/></span>
    </xsl:when>
    <xsl:otherwise>
...
<xsl:template name="insert-index-regular-iref">
  ...
  <xsl:if test="generate-id(.) = generate-id(key('index-item',concat(@item,@anchor))[1])">
    ...
    <xsl:element namespace="http://www.w3.org/1999/xhtml" name="{$item-wrapper-element}" use-attribute-sets="item-wrapper-element">
      ...
      <xsl:if test="$s2">
        <xsl:element namespace="http://www.w3.org/1999/xhtml" name="{$subitems-wrapper-element}">
...
<xsl:template name="insertIndex">
  ...
  <section xmlns="http://www.w3.org/1999/xhtml" id="{$anchor-pref}index">
    <xsl:call-template name="insert-conditional-pagebreak"/>
    <h2>
    ...
    <xsl:when test="@ipr and not($is-rfc)">
      <t>
        <xsl:choose>
          ...
          <xsl:when test="$ipr-2010-01 and (@ipr = 'trust200902'                           or @ipr = 'noModificationTrust200902'                           or @ipr = 'noDerivativesTrust200902'                           or @ipr = 'pre5378Trust200902')">
            This Internet-Draft is submitted in full conformance with
            the provisions of BCP 78 and BCP 79.
          </xsl:when>

          <!-- as of Nov 2008, Feb 2009 and Sep 2009 -->
          <xsl:when test="@ipr = 'trust200811'                           or @ipr = 'noModificationTrust200811'                           or @ipr = 'noDerivativesTrust200811'                           or @ipr = 'trust200902'                           or @ipr = 'noModificationTrust200902'                           or @ipr = 'noDerivativesTrust200902'                           or @ipr = 'pre5378Trust200902'">
  ...
  <xsl:choose>
    ...
    <xsl:when test="$ipr-2008-11">
      <section anchor="{$anchor-pref}copyrightnotice">
        ...
        <xsl:variable name="stds-rfc-compatible-ipr" select="@ipr='pre5378Trust200902' or @ipr='trust200902' or @ipr='trust200811' or @ipr='full3978' or @ipr='full3667' or @ipr='full2026'"/>

        <xsl:variable name="rfc-compatible-ipr" select="$stds-rfc-compatible-ipr or @ipr='noModificationTrust200902' or @ipr='noDerivativesTrust200902' or @ipr='noModificationTrust200811' or @ipr='noDerivativesTrust200811'"/>
                      <!-- TODO: may want to add more historic variants -->

        <xsl:variable name="is-stds-track" select="$submissionType='IETF' and @category='std'"/>
...
<xsl:template match="/" mode="toc">
  <hr xmlns="http://www.w3.org/1999/xhtml" class="{$css-noprint}"/>

  <nav xmlns="http://www.w3.org/1999/xhtml" id="{$anchor-pref}toc">
    <xsl:call-template name="insert-errata">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:if test="$number != '' and not(contains($number,$unnumbered))">
            <a xmlns="http://www.w3.org/1999/xhtml" href="#{$anchor-pref}section.{$number}">
              <xsl:call-template name="emit-section-number">
...
<xsl:template name="rfc-or-id-link">
  ...
  <xsl:choose>
    <xsl:when test="starts-with($name,'draft-')">
      ...
      <a xmlns="http://www.w3.org/1999/xhtml" href="{$uri}"><xsl:value-of select="$name"/></a>
...
<xsl:template match="x:ref">
  ...
  <xsl:choose>
    <xsl:when test="$target[1]/@anchor">
      <a xmlns="http://www.w3.org/1999/xhtml" href="#{$target[1]/@anchor}" class="smpl">
        <xsl:call-template name="copy-anchor"/>
        <!-- insert id when a backlink to this xref is needed in the index -->
    ...
    <xsl:when test="$target[1]/self::x:source">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          ...
          <a xmlns="http://www.w3.org/1999/xhtml" href="{$t}" class="smpl">
            <xsl:value-of select="."/>
          </a>
        </xsl:otherwise>
      </xsl:choose>
...
<xsl:template match="x:bcp14|bcp14">
  ...
  <xsl:choose>
    <xsl:when test="$c='MUST' or $c='REQUIRED' or $c='SHALL' or $c='MUST NOT'       or $c='SHALL NOT' or $c='SHOULD' or $c='RECOMMENDED' or $c='SHOULD NOT'       or $c='NOT RECOMMENDED' or $c='MAY' or $c='OPTIONAL'">
...
<xsl:template match="x:span">
  <xsl:choose>
    <xsl:when test="@x:lang and $prettyprint-class!=''">
      <code xmlns="http://www.w3.org/1999/xhtml" class="{$prettyprint-class}">
        <xsl:call-template name="copy-anchor"/>
        <xsl:apply-templates/>
      </code>
    </xsl:when>
...
<xsl:template match="ed:issueref">
  <xsl:choose>
    <xsl:when test=".=//ed:issue/@name">
      <a xmlns="http://www.w3.org/1999/xhtml" href="#{$anchor-pref}issue.{.}">
        <xsl:apply-templates/>
      </a>
    </xsl:when>
    <xsl:when test="@href">
      <a xmlns="http://www.w3.org/1999/xhtml" href="{@href}" id="{$anchor-pref}issue.{.}">
        <xsl:apply-templates/>
      </a>
    </xsl:when>
    <xsl:otherwise>
...
<xsl:template name="insertIssuesList">
  ...
  <h2 xmlns="http://www.w3.org/1999/xhtml" id="{$anchor-pref}issues-list"><a href="#{$anchor-pref}issues-list">Issues list</a></h2>
  ...
  <xsl:for-each select="@ed:resolves|ed:resolves">
    ...
    <xsl:choose>
      ...
      <xsl:when test="not(ancestor::t) and not(ancestor::title) and not(ancestor::figure) and not($change/@ed:old-title)">
        <div xmlns="http://www.w3.org/1999/xhtml" class="issuepointer {$css-noprint}">
      ...
      <xsl:otherwise>
        <xsl:if test="$count &gt; 1">
          <a xmlns="http://www.w3.org/1999/xhtml" class="bg-issue" title="previous change for {$resolves}" href="#{$anchor-pref}change.{$resolves}.{$count - 1}">↑</a>
        </xsl:if>
        <a xmlns="http://www.w3.org/1999/xhtml" title="resolves: {$resolves}" href="#{$anchor-pref}issue.{$resolves}">
          <xsl:if test="not($deleted-anchor)">
            <xsl:attribute name="id"><xsl:value-of select="$id"/></xsl:attribute>
          </xsl:if>
          <xsl:choose>
        ...
        <xsl:if test="$count &lt; $total">
          <a xmlns="http://www.w3.org/1999/xhtml" class="bg-issue" title="next change for {$resolves}" href="#{$anchor-pref}change.{$resolves}.{$count + 1}">↓</a>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:for-each>
...
<xsl:template name="emit-message-inline">
  ...
  <xsl:choose>
    <xsl:when test="ancestor::t or ancestor-or-self::seriesInfo">
      <span xmlns="http://www.w3.org/1999/xhtml" class="{$css-error}"><xsl:value-of select="$message"/></span>
    </xsl:when>
    <xsl:otherwise>
      <div xmlns="http://www.w3.org/1999/xhtml" class="{$css-error}"><xsl:value-of select="$message"/></div>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>
...
<xsl:template match="table">
  <div xmlns="http://www.w3.org/1999/xhtml" class="{$css-tt}">
    <xsl:call-template name="copy-anchor"/>
...
<xsl:template match="texttable">
  ...
  <div xmlns="http://www.w3.org/1999/xhtml" id="{$anch}" class="{$css-tt}">
...
<xsl:template match="/*/middle//section[not(ancestor::section)]" mode="links">
  ...
  <link xmlns="http://www.w3.org/1999/xhtml" rel="Chapter" title="{$title}" href="#{$anchor-pref}section.{$sectionNumber}"/>
...
<xsl:template match="/*/back//section[not(ancestor::section)]" mode="links">
  ...
  <link xmlns="http://www.w3.org/1999/xhtml" rel="Appendix" title="{$title}" href="#{$anchor-pref}section.{$sectionNumber}"/>
  <xsl:apply-templates mode="links"/>
</xsl:template>
...
<xsl:template name="get-generator">
  <xsl:variable name="gen">
    ...
    <xsl:if test="contains('$Revision: 1.1377 $',':')">
      <xsl:value-of select="concat('Revision ',normalize-space(translate(substring-after('$Revision: 1.1377 $', 'Revision: '),'$','')),', ')"/>
    </xsl:if>
    <xsl:if test="contains('$Date: 2021/03/31 15:23:07 $',':')">
      <xsl:value-of select="concat(normalize-space(translate(substring-after('$Date: 2021/03/31 15:23:07 $', 'Date: '),'$','')),', ')"/>
    </xsl:if>
...
<xsl:template name="u-expanded-name">
  <xsl:param name="lit"/>
  <xsl:choose>
    <xsl:when test="string-length($lit)=0"/>
    <xsl:otherwise>
...
<!-- t element -->
<xsl:template match="abstract/t | abstract/ed:replace/ed:*/t |                      list/t | list/ed:replace/ed:*/t |                      note/t | note/ed:replace/ed:*/t |                      section/t | section/ed:replace/ed:*/t |                      blockquote/t |                      x:blockquote/t | x:blockquote/ed:replace/ed:*/t |                      x:note/t | x:note/ed:replace/ed:*/t |                      aside/t |                      td/t | th/t |                      x:lt/t | li/t | x:lt/ed:replace/ed:*/t | dd/t" mode="validate" priority="9">
  <xsl:apply-templates select="@*|*" mode="validate"/>
</xsl:template>
...
<xsl:template match="a|xhtml:a" mode="strip-links">
  <xsl:choose>
    <xsl:when test="@id">
      <span xmlns="http://www.w3.org/1999/xhtml" id="{@id}">
      	<xsl:apply-templates select="node()" mode="strip-links"/>
      </span>
    </xsl:when>
    <xsl:otherwise>
